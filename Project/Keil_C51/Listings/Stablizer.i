...
#line 1 "..\..\SrcCode\Logic\Core\Stablizer.c" /0
...
#line 2 "\PgmCard\SrcCode\Logic\Inc\Stablizer.h" /1
...
#line 1 "..\..\SrcCode\Logic\Core\Stablizer.c" /0
 ...
 uint8_t  RxTxStatue;
 int16_t  StabilizerParam[18];
 
 
 uint8_t  RxDatHandle;
 
 uint8_t  RxStatusIdx;
 uint8_t  RxPackageData;
 uint8_t  RxPackageLength, RxPackageCnt;
 
 uint8_t  TxSocketDat[(0x0A + 0x03)];
 uint8_t  RxSocketDat[(0x0A + 0x03)]; 
 
 uint8_t  data DetectStablizerTickOverFlow;
 uint16_t data DetectStablizerTickCnt;
 
 uint8_t  data StablizerTickOverFlow;
 uint16_t data StablizerTickCnt;
 ...
 TxSocketDat[0] = 0x8D; 
 TxSocketDat[1] = 0x0A;
 TxSocketDat[2] = StabilizerParam[1]   +   StabilizerParam[15] 	* 100;
 TxSocketDat[3] = StabilizerParam[2]  +   StabilizerParam[16] * 100;
 TxSocketDat[4] = StabilizerParam[3]    +   StabilizerParam[14] 	* 100;
 TxSocketDat[5] = StabilizerParam[4]    +   StabilizerParam[15] 	* 100;
 TxSocketDat[6] = StabilizerParam[5]   +   StabilizerParam[16] * 100;
 TxSocketDat[7] = StabilizerParam[6]     +   StabilizerParam[14] 	* 100;
 TxSocketDat[8] = StabilizerParam[7]  +   20;
 TxSocketDat[9] = StabilizerParam[8] +   20;
 
 TxSocketDat[10] = (StabilizerParam[9] << 0) | (StabilizerParam[10] << 2)
 | (StabilizerParam[11] << 4) | (StabilizerParam[13] << 6);
 
 
 TxSocketDat[11] = (StabilizerParam[17] << 4)		
 |	(StabilizerParam[0] << 2) | (StabilizerParam[12] << 0);
 ...
 if(RxSocketDat[2] > 100) { StabilizerParam[15] = 1; }
 else if(RxSocketDat[2] <= 100) { StabilizerParam[15] = 0; }
 
 if(RxSocketDat[3] > 100) { StabilizerParam[16] = 1; }
 else if(RxSocketDat[3] <= 100) { StabilizerParam[16] = 0; }
 
 if(RxSocketDat[4] > 100) { StabilizerParam[14] = 1; }
 else if(RxSocketDat[4] <= 100) { StabilizerParam[14] = 0; }
 
 StabilizerParam[1] 	= RxSocketDat[2] - StabilizerParam[15] 	* 100;
 StabilizerParam[4] 		= RxSocketDat[5] - StabilizerParam[15] 	* 100; 	
 
 StabilizerParam[2] 	= RxSocketDat[3] - StabilizerParam[16] * 100;
 StabilizerParam[5] 	= RxSocketDat[6] - StabilizerParam[16] * 100; 
 
 StabilizerParam[3] 		= RxSocketDat[4] - StabilizerParam[14] 	* 100;
 StabilizerParam[6] 		= RxSocketDat[7] - StabilizerParam[14] 	* 100;
 
 StabilizerParam[7] 	= RxSocketDat[8] - 20;
 StabilizerParam[8] = RxSocketDat[9] - 20;
 
 StabilizerParam[9] 				= (RxSocketDat[10] >> 0) & 0x03;
 StabilizerParam[10] 				= (RxSocketDat[10] >> 2) & 0x03;
 StabilizerParam[11] 				= (RxSocketDat[10] >> 4) & 0x03;
 StabilizerParam[13] 					= (RxSocketDat[10] >> 6) & 0x03;
 
 StabilizerParam[12] 		= (RxSocketDat[11] >> 0) & 0x03;
 StabilizerParam[0] 		= (RxSocketDat[11] >> 2) & 0x03;
 StabilizerParam[17]				= (RxSocketDat[11] >> 4) & 0x03;
 }
 ...
 if(RxSocketDat[0] == 0xD8 && RxSocketDat[1] == 0x01 && RxSocketDat[2] == 0xCC){	 
 RxTxStatue &= ~0x02;
 ...
 SumPre = Sum;
 if(!(RxTxStatue&0x02)){
 RxTxStatue &= ~0x01; RxTxStatue |= 0x02;
 ...
 for(i = 0; i < (0x0A + 0x03); i++) TxSocketDat[i] = 0;
 for(i = 0; i < (0x0A + 0x03); i++) RxSocketDat[i] = 0;
 for(i = 0; i < 18; i++) StabilizerParam[i] = 0;	
 ...
 if(RxDatHandle == 1){ 
 SysModuleEnable |= 0x01;
 return;
 }
 
  TCON_IT1 = 0;
 if(DetectStablizerTickOverFlow == 0x1){
 if(TickCnt1ms > (0xFFFF)-0x04){
 ...
 P1MDOUT = (P1MDOUT&(~0x01))|0x00;
 P1MDIN = (P1MDIN&(~0x01))|0x01;
 P1_B0 = 0x1; 
 
 P1MDOUT = (P1MDOUT&(~0x02))|0x00;
 P1MDIN = (P1MDIN&(~0x02))|0x02;
 P1_B1 = 0x1;
 }
 ...
 if(RxTxStatue&0x01){
 UART1_Read_Encode();
 UART1_Write_Buffer(TxSocketDat, 0x01+0x03);
 }else if(RxTxStatue&0x02){
 UART1_Write_Encode();
 UART1_Write_Buffer(TxSocketDat, 0x0A+0x03);
 }
 }
 }
 }
