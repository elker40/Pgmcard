...
#line 1 "..\..\SrcCode\Logic\Core\Sterring.c" /0
...
#line 1 "..\..\SrcCode\Logic\Core\Sterring.c" /0
 ...
 sint16_t  SterringParam[3];
 
 uint16_t data SterringPwmCnt;
 uint16_t data SterringTickTime;
 uint16_t data SterringPwmPulse;
 uint16_t data SterringPwmPeriod;
 
 uint8_t data SterringStepState;
 ...
 SterringPwmCnt ++;
  TMR3CN0 = (TMR3CN0&(~0x80))|0x00;;     
 
 if(0x00 == SterringParam[0x00]){  
 SterringTickTime++;
 if(SterringTickTime > 500+(2000-40*SterringParam[0x02])){  
 if(0x00 != SterringStepState){
 SterringParam[0x01] ++ ;
 if(SterringParam[0x01] == +50) SterringStepState = 0x00;
 }else{
 SterringParam[0x01] --;
 if(SterringParam[0x01] == -50) SterringStepState = 0x01; 
 }
 SterringPwmPulse = SterringParam[0x01] + 150;			
 SterringMs.DrawMask = PD_LIST|PD_IDX;
 SterringTickTime = 0x0000;
 }
 }else if(0x01 == SterringParam[0x00]){	 
 SterringPwmPulse = SterringParam[0x01] + 150;
 }else if(0x02 == SterringParam[0x00]){	 
 SterringParam[0x01] = 0; SterringPwmPulse = SterringParam[0x01] + 150;
 }else if(0x03 == SterringParam[0x00]){	 
 SterringParam[0x01] = +50; SterringPwmPulse = SterringParam[0x01] + 150;
 }else if(0x04 == SterringParam[0x00]){	 
 SterringParam[0x01] = -50; SterringPwmPulse = SterringParam[0x01] + 150;
 }
 }
