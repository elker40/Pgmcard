...
#line 1 "..\..\SrcCode\Middle\Gui\Core\Page.c" /0
...
#line 1 "..\..\SrcCode\Middle\Gui\Core\Page.c" /0
 ...
 uint8_t data PageStackIdx;
 PageFun data PageStack[3];
 
 void PageInit(PageFun Page, uint8_t Msg){
 for(PageStackIdx = 0; PageStackIdx < 3; PageStackIdx ++){
 PageStack[PageStackIdx] = ((void *)0);
 }    
 PageStackIdx = 0;
 ...
 uint8_t data i = 0, idx = 0; 
 uint16_t data s = 0, p = 0, x = 0, y =0;
 MenuItem *mi = (MenuItem *)&pmi[pms->Focus+1]; 
 
 if(pms->DrawMask){ 
 if(pms->DrawMask&PD_TITLE){		
 LcdDrawRect(0, 0, 128-1, 15, 0); 
 LcdDrawTextString(3, 1, pmi[0].Title); LcdDrawHLine(0, 128-1, 15, 1);
 }
 
 if(pms->DrawMask&PD_IDX){
 if(pms->Total > 9){
 LcdDrawMiniInt(128-12, 6, pms->Focus+1, 2, 0, 0xFF, 1);    
 LcdDrawMiniBmp(128-12, 6, 17);                     
 LcdDrawMiniInt(128-8, 6, pms->Total, 2, 0, 0xFF, 0);       
 }else{
 LcdDrawMiniInt(128-12, 6, pms->Focus+1, 2, 0, 0xFF, 1);    
 LcdDrawMiniBmp(128-12, 6, 17);                     
 LcdDrawMiniInt(128-8, 6, pms->Total, 1, 0, 0xFF, 0);       
 }
 }
 
 if(pms->DrawMask&(PD_LIST|PD_VALUE)){
 for(i = 0, y = 16; i < 3 && i < pms->Total; i++, y+=16){
 idx = pms->Start + i;
 LcdBw = (idx == pms->Focus);
 idx++;
 ...
 LcdDrawRect(1, y+1, 128-1, y+15, LcdBw);	   
 LcdDrawMiniInt(2, y+6, idx, 0, 0, 0xFF, 0);	   
 LcdDrawMiniBmp(x+6, y+6, 10);		       
 LcdDrawTextString(x+8, y+2, pmi[idx].Title);   
 
 if(pmi[idx].SubItem){
 if(LcdBw){
 LcdDrawTextString(128-14, y+2, "\6\2");  
 }else{
 LcdDrawTextString(128-8, y+2, "\2");     
 }
 }
 
 if(pmi[idx].Text){		                         
 s = (128-10)-6*strlen(pmi[idx].Text);
 p = LcdDrawTextString(s, y+2, pmi[idx].Text);
 LcdDrawHLine(s, p, y+14, 0);                 
 }else if(pmi[idx].pValue){	
 if(*pmi[idx].pValue > pmi[idx].Max) *pmi[idx].pValue = pmi[idx].Max;   
 if(*pmi[idx].pValue < pmi[idx].Min) *pmi[idx].pValue = pmi[idx].Min;   
 
 if(pmi[idx].ValueList){                      
 s = (128-10)-6*strlen(pmi[idx].ValueList[*pmi[idx].pValue]);
 p = LcdDrawTextString(s , y+2, (char *)pmi[idx].ValueList[*pmi[idx].pValue]);
 }else{  
 LcdDrawInt(s = 128-38, y+2, *pmi[idx].pValue, 3, pmi[idx].Data, 0, 0); 
 p = (128-38)+24+2;
 }
 }
 
 if(pms->ItemEdit&&LcdBw){  									 
 if(*pmi[idx].pValue == pmi[idx].Max){
 LcdDrawTextString(128-8, y+2, "\4");   
 }else if(*pmi[idx].pValue == pmi[idx].Min){
 LcdDrawTextString(128-8, y+2, "\5");   
 }else{
 LcdDrawTextString(128-8, y+2, "\3");   
 ...
 if(pms->Focus < pms->Start) pms->Start = pms->Focus;   
 if(pms->Focus > pms->Start+3-1) pms->Start = pms->Focus-(3-1);  
 } 
